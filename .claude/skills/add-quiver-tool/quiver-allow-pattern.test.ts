 ...
 *
 * Registering `quiver` in a group's mcpServers map is the *only* wiring needed to expose
 * `mcp__quiver__*` to the agent — there is no static TOOL_ALLOWLIST edit. That holds solely
 ...
 * so we assert the derivation structurally. Delete or rename it and this goes red —
 * surfacing that `quiver` tools would silently be filtered out despite being registered.
 *
 * `mcpAllowPattern`'s mapping is exercised directly to prove `quiver` -> `mcp__quiver__*`.
 */
import fs from 'fs';
...
/** Reimplement the sanitizer the provider applies, to assert the quiver name maps cleanly. */
...
describe('claude.ts derives MCP allow-patterns from the registered servers', () => {
  ...
  it('maps a quiver server name to mcp__quiver__*', () => {
    expect(expectedPattern('quiver')).toBe('mcp__quiver__*');
  });
});
